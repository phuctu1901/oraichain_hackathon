...
use crate::msg::{HandleMsg, InitMsg, MintMsg, MinterResponse, QueryMsg, ExpertResponseMsg};
use crate::state::{
...
pub fn handle<S: Storage, A: Api, Q: Querier>(
    ...
    match msg {
        ...
        HandleMsg::AddExpertResponse(msg) => handle_add_expert_response(deps, env, info, msg),
...
pub fn handle_add_expert_response<S: Storage, A: Api, Q: Querier>(
    deps: &mut Extern<S, A, Q>,
    _env: Env,
    info: MessageInfo,
    msg: ExpertResponseMsg,
) -> Result<HandleResponse, ContractError> {
    let minter = MINTER.load(&deps.storage)?;
    let sender_raw = deps.api.canonical_address(&info.sender)?;
    ...
    let transaction = TransactionInfo {
        ...
        expert_output_data: msg.expert_output_data,
    };
