...
const aioracle_handle = async (data) => {
    ...
    // const service = "ThoracicDiseasesDetectionAndSegmentation";
    ...
    const response_data = JSON.stringify(reports);
    const base64 = JSON.parse(response_data).data[0].report.data;

    let text = Buffer.from(base64, 'base64').toString('ascii')

    // return result;
    const output_data = JSON.parse(JSON.parse(JSON.parse(text)));

    return {txHash:txHash, data: output_data };
...
app.post('/aioracle', async (req, res) => {
    const data = req.body;
    const result = await aioracle_handle(data)
...
app.listen(port, () => console.log(`AI Oracle API listening on ${port}!`));